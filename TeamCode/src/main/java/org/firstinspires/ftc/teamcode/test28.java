...
@Configurable
@TeleOp(name = "test28")
public class test28 extends OpMode {
    ...
    public static double LL_MICRO_kP = 0.015;     // VERY SMALL
    public static double LL_MICRO_MAX = 0.15;     // clamp correction
    public static double LL_MICRO_DEADBAND = 0.4; // degrees (TX)
    ...
    // ================= TURRET AUTO (PEDRO + LIMELIGHT MICRO) =================
    private void alignTurret() {

        double power;

        /* ---------------- PEDRO FIELD-CENTRIC AIM ---------------- */
        ...
        if (ll != null && ll.isValid()) {
            ...
            double tx = ll.getTx(); // horizontal error in degrees

            if (Math.abs(tx) > LL_MICRO_DEADBAND) {
                double llCorrection = tx * LL_MICRO_kP;
                llCorrection = clamp(llCorrection, -LL_MICRO_MAX, LL_MICRO_MAX);
                power += llCorrection;
            }
        }
