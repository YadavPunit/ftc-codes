...
@Configurable
@TeleOp(name = "test11")
public class test11 extends OpMode {
    ...
    // 🔥 ALIGN NOW USES TX (ROTATED LIMELIGHT)
    static final double ALIGN_DEADBAND = 0.3;
    static final double MAX_ALIGN_ERROR = 8.0;
    static final double MAX_TURN = 0.45;

    static final double INTAKE_POWER = -0.6;
    static final double SLOW_MODE = 0.5;

    static final double TX_ALPHA = 0.25;
    static final double TY_ALPHA = 0.20;

    static final double TX_SHORT_MIN = -4.97;
    static final double TX_SHORT_MAX = 14.0;
    static final double RPM_SHORT_MIN = 2950;
    static final double RPM_SHORT_MAX = 3600;
    ...
    static final double RPM_LONG_MIN = 4250;
    static final double RPM_LONG_MAX = 4500;
    static final double HOOD_LONG_MIN = 0.30;
    static final double HOOD_LONG_MAX = 0.30;
    ...
    public void loop() {
        ...
        if (limelightStarted && now - lastLLTime > LL_DT) {
            ...
            if (ll != null && ll.isValid()) {
                filteredTx += TX_ALPHA * (ll.getTx() - filteredTx);
                filteredTy += TY_ALPHA * (ll.getTy() - filteredTy);
        ...
        // ===== ✅ FIXED BOT ALIGN (TX BASED) =====
        if (alignBot && cachedHasTarget) {

            double error = filteredTx; // 🔥 FIX HERE
    ...
    private void updateAutoShot(double tx) {
        ...
        if (tx <= TX_SHORT_MAX) {
            targetRPM = lerp(TX_SHORT_MIN, TX_SHORT_MAX,
                    RPM_SHORT_MIN, RPM_SHORT_MAX,
                    clamp(tx, TX_SHORT_MIN, TX_SHORT_MAX));
            hoodPos = lerp(TX_SHORT_MIN, TX_SHORT_MAX,
                    HOOD_SHORT_MIN, HOOD_SHORT_MAX,
                    clamp(tx, TX_SHORT_MIN, TX_SHORT_MAX));
        }
        else if (tx < TX_LONG_MIN) {
            targetRPM = lerp(TX_SHORT_MAX, TX_LONG_MIN,
                    RPM_SHORT_MAX, RPM_LONG_MIN, tx);
            hoodPos = lerp(TX_SHORT_MAX, TX_LONG_MIN,
                    HOOD_SHORT_MAX, HOOD_LONG_MIN, tx);
        }
        else {
            targetRPM = lerp(TX_LONG_MIN, TX_LONG_MAX,
                    RPM_LONG_MIN, RPM_LONG_MAX,
                    clamp(tx, TX_LONG_MIN, TX_LONG_MAX));
            hoodPos = lerp(TX_LONG_MIN, TX_LONG_MAX,
                    HOOD_LONG_MIN, HOOD_LONG_MAX,
                    clamp(tx, TX_LONG_MIN, TX_LONG_MAX));
        }

        shooterL.setVelocity(rpmToTicks(targetRPM));
        shooterR.setVelocity(rpmToTicks(targetRPM));
