...
@Configurable
@TeleOp(name = "test23_turet_logic2")
public class test23_turet_logic2 extends OpMode {
...
// ===== LIMIT WRAP / RECENTER LOGIC =====
        if ((enable || Math.abs(lastTurnInput) > 0.05) &&
                pos >= TURRET_MAX_TICKS && power > 0) {
            power = -TURRET_RECENTER_POWER;
            recentering = true;
        }
        else if ((enable || Math.abs(lastTurnInput) > 0.05) &&
                pos <= TURRET_MIN_TICKS && power < 0) {
            power = TURRET_RECENTER_POWER;
            recentering = true;
        }

        if (recentering) {
            lastTurretPower = power; // reset slew immediately
        }
