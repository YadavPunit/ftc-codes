...
@Configurable
@TeleOp(name = "test29")
public class test29 extends OpMode {
    ...
    public static double LL_MICRO_kP = 0.02;      // Increased slightly for standalone tracking
    public static double LL_MICRO_MAX = 0.4;      // Allow more power when active
    public static double LL_MICRO_DEADBAND = 0.2; // tighter deadband
    ...
    private boolean forceIntakeOn = false; // To handle preset override
    ...
    public void init() {
        ...
        limelight.pipelineSwitch(1); // Force Pipeline 1 (AprilTag)
        limelight.start();
    ...
    public void loop() {
        ...
        // ===== TURRET ALIGNMENT MANAGER =====
        // Logic: Pedro is active by default.
        // If (Left Bumper AND Tag Visible), Limelight is active.
        alignTurret();
        ...
        if (gamepad1.left_trigger > 0.05) {
            ...
            triggerActive = true;
            forceIntakeOn = false; // Reset forced state on manual input
        ...
        // 2. Check Toggles/Presets
        if (triggerActive) {
            intake.setPower(intakePower);
        } else {
            // Manual Toggle (Gamepad 2 Cross)
            if (gamepad2.cross && !prevX) {
                intakeToggle = !intakeToggle;
                forceIntakeOn = false; // Manual toggle overrides preset
            }
        ...
        // MANUAL LIMELIGHT SHOOTING (Gamepad 2 Left Bumper)
        if (gamepad2.left_bumper) {
            LLResult ll = limelight.getLatestResult();
            if (ll != null && ll.isValid()) {
                double tx = ll.getTx();
                double[] mapData = getInterpolatedMap(tx); // Helper at bottom
                runShooterRPM(mapData[0]);
                hood.setPosition(mapData[1]);
            }
        }
        // PRESET: SHORT (Triangle)
        else if (gamepad2.triangle) {
            runShooterRPM(SHORT_RPM);
            hood.setPosition(SHORT_HOOD);
            forceIntakeOn = true; // Activate Intake
        }
        // PRESET: LONG (Circle)
    ...
    // ================= TURRET ALIGNMENT (STATE MACHINE) =================
    private void alignTurret() {
        ...
        // 2. Determine Mode
        // Active ONLY if Bumper Pressed AND Tag is Visible (Safe Mechanism)
        boolean useLimelight = gamepad1.left_bumper && tagVisible;

        if (useLimelight) {
            // --- LIMELIGHT PID ---
            // If we are here, tagVisible is definitely true
            double tx = ll.getTx();
            if (Math.abs(tx) > LL_MICRO_DEADBAND) {
                power = tx * LL_MICRO_kP;
                telemetryM.addData("Align", "LIMELIGHT (TX: " + tx + ")");
            }
        } else {
            // --- PEDRO FIELD-CENTRIC PID (DEFAULT) ---
            ...
            // Target Blue Goal
            double fieldAngleDeg = Math.toDegrees(Math.atan2(131 - botY, 14 - botX));
            ...
            telemetryM.addData("Align", "PEDRO (Err: " + error + ")");
        }
        ...
        if ((power > 0 && pos >= TURRET_MAX_TICKS) || (power < 0 && pos <= TURRET_MIN_TICKS)) {
            power = 0;
        }
    ...
    private double[] getInterpolatedMap(double tx) {
        double mappedRPM, mappedHood;
        if (tx >= 15.77)      { mappedRPM = 3820; mappedHood = 0.25; }
        else if (tx >= 14.18) { mappedRPM = map(tx, 14.18, 15.77, 3360, 3630); mappedHood = map(tx, 14.18, 15.77, 0.28, 0.25); }
        else if (tx >= 13.18) { mappedRPM = map(tx, 13.18, 14.18, 3200, 3350); mappedHood = map(tx, 13.18, 14.18, 0.39, 0.28); }
        else if (tx >= 11.92) { mappedRPM = map(tx, 11.92, 13.18, 3050, 3200); mappedHood = map(tx, 11.92, 13.18, 0.41, 0.39); }
        else if (tx >= 9.96)  { mappedRPM = map(tx, 9.96, 11.92, 2900, 3050); mappedHood = map(tx, 9.96, 11.92, 0.45, 0.41); }
        else if (tx >= 8.08)  { mappedRPM = map(tx, 8.08, 9.96, 2800, 2900); mappedHood = map(tx, 8.08, 9.96, 0.49, 0.45); }
        else if (tx >= 6.42)  { mappedRPM = map(tx, 6.42, 8.08, 2660, 2800); mappedHood = map(tx, 6.42, 8.08, 0.52, 0.49); }
        else if (tx >= 3.50)  { mappedRPM = map(tx, 3.50, 6.42, 2600, 2660); mappedHood = map(tx, 3.50, 6.42, 0.60, 0.52); }
        else if (tx >= 1.14)  { mappedRPM = map(tx, 1.14, 3.50, 2540, 2600); mappedHood = map(tx, 1.14, 3.50, 0.75, 0.60); }
        else                  { mappedRPM = 2470; mappedHood = 0.89; }
        return new double[]{mappedRPM, mappedHood};
    }
}